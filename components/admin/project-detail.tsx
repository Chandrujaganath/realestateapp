...
export function ProjectDetail({ projectId }: ProjectDetailProps) {
  ...
  const _handleGridSave = async (gridData: { rows: number; cols: number; cells: GridCell[] }) => {
    try {
      const docRef = doc(db, 'projects', projectId);
      ...
      const plotCounts = {
        ...
        available: gridData.cells.filter(
          (cell) => cell.type === 'plot' && cell.status === 'available'
        ).length,
        reserved: gridData.cells.filter(
          (cell) => cell.type === 'plot' && cell.status === 'reserved'
        ).length,
        sold: gridData.cells.filter((cell) => cell.type === 'plot' && cell.status === 'sold')
          .length,
        pending: gridData.cells.filter((cell) => cell.type === 'plot' && cell.status === 'pending')
          .length,
      };
    ...
    <div className="space-y-6">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            ...
            <div className="flex flex-col">
              ...
              <span>
                {project.totalPlots || 0} plots ({project.availablePlots || 0} available)
              </span>
            </div>
          </div>
      ...
      <Tabs defaultValue="grid">
        ...
        <TabsContent value="managers">
          <ManagerAssignment
            projectId={projectId}
            initialManagers={project.managersAssigned || []}
          />
        </TabsContent>
