...
export function AnnouncementForm({ announcement, mode }: AnnouncementFormProps) {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      expiresAt: announcement?.expiresAt ? new Date(announcement.expiresAt) : new Date(Date.now() + 7 * 24 * 60 * 60 * 1000), // Default to 7 days from now
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      if (mode === "create") {
        ...
        const announcementId = await createAnnouncement(formattedValues)
        if (announcementId) {
          router.push("/admin/announcements")
        }
      } else if (mode === "edit" && announcement) {
        // Update existing announcement
        const success = await updateAnnouncement(announcement.id, formattedValues)
        if (success) {
          router.push("/admin/announcements")
        }
      }
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>
                    A clear, concise title for the announcement.
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
                      ...
                      {priorityOptions.map((option) => (
                        <div key={option.id} className="flex items-center space-x-2">
                          ...
                          <Label htmlFor={`priority-${option.id}`} className="font-normal">
                            {option.label} - <span className="text-muted-foreground text-sm">{option.description}</span>
                          </Label>
                        </div>
                      ))}
                    </RadioGroup>
