...
import { doc, getDoc, setDoc, serverTimestamp, updateDoc, arrayUnion, query, where, orderBy, collection, getDocs, addDoc, deleteDoc, Firestore, CollectionReference, Query } from "firebase/firestore";
...
export const AuthProvider: React.FC<{ children?: React.ReactNode }> = ({ children }) => {
  ...
  const getAnnouncements = async () => {
    try {
      ...
      // Create the query
      const announcementsRef = collection(db as Firestore, "announcements");
  ...
  const createAnnouncement = async (data: any): Promise<string | void> => {
    try {
      ...
      const docRef = await addDoc(collection(db as Firestore, "announcements"), announcementData);
      return docRef.id;
    } catch (error) {
...
export function useAuth() {
  ...
  if (context === undefined) {
    // In development, log error but provide fallback empty context
    if (process.env.NODE_ENV === 'development') {
      console.error("useAuth must be used within an AuthProvider. Providing fallback empty context.");
      
      // Return a fallback context with empty values to prevent crashes
      return {
        user: null,
        loading: false,
        ...
        // Include all other required properties with fallback implementations
        signUpAsGuest: async () => {
          console.error("Auth not initialized");
          throw new Error("AuthContext not available");
        },
