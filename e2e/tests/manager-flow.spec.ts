...
test.describe("Manager User Flow", () => {
  test("complete manager journey", async ({ page }) => {
    ...
    await page.fill('input[name="startDate"]', new Date(Date.now() + 604800000).toISOString().split("T")[0]) // Next week
    await page.fill('input[name="endDate"]', new Date(Date.now() + 1209600000).toISOString().split("T")[0]) // Two weeks from now
