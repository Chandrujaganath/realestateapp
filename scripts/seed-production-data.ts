...
async function createProjectsAndPlots(users) {
  ...
  for (let i = 0; i < projectNames.length; i++) {
    ...
    const project = {
      ...
      features: ["Swimming Pool", "Fitness Center", "Security System", "Landscaped Gardens", "Community Center"].slice(
        0,
    ...
    for (let j = 0; j < 20; j++) {
      ...
      const plot = {
        ...
        features: ["Corner Plot", "Park Facing", "Wide Road", "Near Entrance", "Premium Location"].slice(
          0,
...
async function createVisits(users, projects) {
  ...
  for (let i = 0; i < 50; i++) {
    ...
    const guestUser = guestUsers[Math.floor(Math.random() * guestUsers.length)]
    const project = projects[Math.floor(Math.random() * projects.length)]
    const isCompleted = Math.random() > 0.3
    const isApproved = Math.random() > 0.2
    const managerUser = managerUsers[Math.floor(Math.random() * managerUsers.length)]

    const visitDate = getRandomFutureDate(15)
    ...
    if (isApproved && visit.qrCode) {
      try {
        const storageRef = ref(storage, `qrcodes/visits/${visitId}.png`)
...
async function createAnnouncements(users) {
  ...
  for (let i = 0; i < 15; i++) {
    ...
    const announcement = {
      ...
      targetRoles: Math.random() > 0.5 ? ["all"] : ["client", "manager"].slice(0, Math.floor(Math.random() * 2) + 1),
    }
...
async function createTasks(users, projects) {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    const task = {
      id: taskId,
      title: ["Client Meeting", "Site Inspection", "Document Verification", "Follow-up Call", "Report Preparation"][
        Math.floor(Math.random() * 5)
      ],
