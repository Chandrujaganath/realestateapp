...
export async function authenticateRequest(req: NextRequest) {
  ...
  try {
    ...
    if (error instanceof Error && error.message.includes("credential")) {
      console.error("[API-AUTH] This may be due to missing Firebase Admin credentials. Check FIREBASE_SERVICE_ACCOUNT_KEY in your environment variables.");
    }
    return { 
      authenticated: false, 
      response: NextResponse.json({ error: "Unauthorized", details: process.env.NODE_ENV === 'development' ? (error instanceof Error ? error.message : String(error)) : undefined }, { status: 401 })
    };
  }
} 