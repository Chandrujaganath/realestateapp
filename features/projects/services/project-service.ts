import { where, orderBy, limit, _query, _collection, _getDocs } from 'firebase/firestore';
...
export class ProjectService extends FirebaseService {
  ...
  async getProjectsByManagerId(managerId: string): Promise<Project[]> {
    return this.queryDocs<Project>(this.projectsCollection, [
  ...
  async getPlotsByProjectId(projectId: string): Promise<Plot[]> {
    return this.queryDocs<Plot>(this.plotsCollection, [
  ...
  async createPlot(payload: CreatePlotPayload): Promise<string> {
    ...
    const _plotId = await this.addDoc<CreatePlotPayload & { status: string }>(
