...
export function useProjects(filterBy?: { status?: string; location?: string }) {
   ...
   * Create a new plot
   */
  const createPlot = useCallback(async (payload: CreatePlotPayload) => {
    setLoading(true);
    try {
      const plotId = await projectService.createPlot(payload);
   ...
   * Update a plot
   */
  const updatePlot = useCallback(async (id: string, payload: UpdatePlotPayload) => {
    setLoading(true);
    try {
