...
declare const _CHILD_MESSAGE_CALL = 1;

declare const _CHILD_MESSAGE_CALL_SETUP = 4;

declare const _CHILD_MESSAGE_END = 2;

declare const _CHILD_MESSAGE_INITIALIZE = 0;

declare const _CHILD_MESSAGE_MEM_USAGE = 3;
...
declare type ChildMessageEnd = [type: typeof CHILD_MESSAGE_END, isProcessed: boolean];
...
declare type ComputeTaskPriorityCallback = (method: string, ...args: Array<unknown>) => number;
...
export declare type JestWorkerFarm<T extends Record<string, unknown>> = Worker_2 & WorkerModule<T>;

export declare function messageParent(message: unknown, parentProcess?: NodeJS.Process): void;
...
declare type OnStateChangeHandler = (state: WorkerStates, oldState: WorkerStates) => void;
...
declare type WorkerModule<T> = {
  [K in keyof T as Extract<ExcludeReservedKeys<K>, MethodLikeKeys<T>>]: T[K] extends FunctionLike
    ? Promisify<T[K]>
    : never;
};
