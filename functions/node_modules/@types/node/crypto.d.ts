...
declare module 'crypto' {
  ...
  namespace constants {
    ...
    const _OPENSSL_VERSION_NUMBER: number;
    /** Applies multiple bug workarounds within OpenSSL. See https://www.openssl.org/docs/man1.0.2/ssl/SSL_CTX_set_options.html for detail. */
    const _SSL_OP_ALL: number;
    /** Allows legacy insecure renegotiation between OpenSSL and unpatched clients or servers. See https://www.openssl.org/docs/man1.0.2/ssl/SSL_CTX_set_options.html. */
    const _SSL_OP_ALLOW_UNSAFE_LEGACY_RENEGOTIATION: number;
    /** Attempts to use the server's preferences instead of the client's when selecting a cipher. See https://www.openssl.org/docs/man1.0.2/ssl/SSL_CTX_set_options.html. */
    const _SSL_OP_CIPHER_SERVER_PREFERENCE: number;
    /** Instructs OpenSSL to use Cisco's version identifier of DTLS_BAD_VER. */
    const _SSL_OP_CISCO_ANYCONNECT: number;
    /** Instructs OpenSSL to turn on cookie exchange. */
    const _SSL_OP_COOKIE_EXCHANGE: number;
    /** Instructs OpenSSL to add server-hello extension from an early version of the cryptopro draft. */
    const _SSL_OP_CRYPTOPRO_TLSEXT_BUG: number;
    /** Instructs OpenSSL to disable a SSL 3.0/TLS 1.0 vulnerability workaround added in OpenSSL 0.9.6d. */
    const _SSL_OP_DONT_INSERT_EMPTY_FRAGMENTS: number;
    /** Instructs OpenSSL to always use the tmp_rsa key when performing RSA operations. */
    const _SSL_OP_EPHEMERAL_RSA: number;
    /** Allows initial connection to servers that do not support RI. */
    const _SSL_OP_LEGACY_SERVER_CONNECT: number;
    const _SSL_OP_MICROSOFT_BIG_SSLV3_BUFFER: number;
    const _SSL_OP_MICROSOFT_SESS_ID_BUG: number;
    /** Instructs OpenSSL to disable the workaround for a man-in-the-middle protocol-version vulnerability in the SSL 2.0 server implementation. */
    const _SSL_OP_MSIE_SSLV2_RSA_PADDING: number;
    const _SSL_OP_NETSCAPE_CA_DN_BUG: number;
    const _SSL_OP_NETSCAPE_CHALLENGE_BUG: number;
    const _SSL_OP_NETSCAPE_DEMO_CIPHER_CHANGE_BUG: number;
    const _SSL_OP_NETSCAPE_REUSE_CIPHER_CHANGE_BUG: number;
    /** Instructs OpenSSL to disable support for SSL/TLS compression. */
    const _SSL_OP_NO_COMPRESSION: number;
    const _SSL_OP_NO_QUERY_MTU: number;
    /** Instructs OpenSSL to always start a new session when performing renegotiation. */
    const _SSL_OP_NO_SESSION_RESUMPTION_ON_RENEGOTIATION: number;
    const _SSL_OP_NO_SSLv2: number;
    const _SSL_OP_NO_SSLv3: number;
    const _SSL_OP_NO_TICKET: number;
    const _SSL_OP_NO_TLSv1: number;
    const _SSL_OP_NO_TLSv1_1: number;
    const _SSL_OP_NO_TLSv1_2: number;
    const _SSL_OP_PKCS1_CHECK_1: number;
    const _SSL_OP_PKCS1_CHECK_2: number;
    /** Instructs OpenSSL to always create a new key when using temporary/ephemeral DH parameters. */
    const _SSL_OP_SINGLE_DH_USE: number;
    /** Instructs OpenSSL to always create a new key when using temporary/ephemeral ECDH parameters. */
    const _SSL_OP_SINGLE_ECDH_USE: number;
    const _SSL_OP_SSLEAY_080_CLIENT_DH_BUG: number;
    const _SSL_OP_SSLREF2_REUSE_CERT_TYPE_BUG: number;
    const _SSL_OP_TLS_BLOCK_PADDING_BUG: number;
    const _SSL_OP_TLS_D5_BUG: number;
    /** Instructs OpenSSL to disable version rollback attack detection. */
    const _SSL_OP_TLS_ROLLBACK_BUG: number;
    const _ENGINE_METHOD_RSA: number;
    const _ENGINE_METHOD_DSA: number;
    const _ENGINE_METHOD_DH: number;
    const _ENGINE_METHOD_RAND: number;
    const _ENGINE_METHOD_EC: number;
    const _ENGINE_METHOD_CIPHERS: number;
    const _ENGINE_METHOD_DIGESTS: number;
    const _ENGINE_METHOD_PKEY_METHS: number;
    const _ENGINE_METHOD_PKEY_ASN1_METHS: number;
    const _ENGINE_METHOD_ALL: number;
    const _ENGINE_METHOD_NONE: number;
    const _DH_CHECK_P_NOT_SAFE_PRIME: number;
    const _DH_CHECK_P_NOT_PRIME: number;
    const _DH_UNABLE_TO_CHECK_GENERATOR: number;
    const _DH_NOT_SUITABLE_GENERATOR: number;
    const _ALPN_ENABLED: number;
    const _RSA_PKCS1_PADDING: number;
    const _RSA_SSLV23_PADDING: number;
    const _RSA_NO_PADDING: number;
    const _RSA_PKCS1_OAEP_PADDING: number;
    const _RSA_X931_PADDING: number;
    const _RSA_PKCS1_PSS_PADDING: number;
    /** Sets the salt length for RSA_PKCS1_PSS_PADDING to the digest size when signing or verifying. */
    const _RSA_PSS_SALTLEN_DIGEST: number;
    /** Sets the salt length for RSA_PKCS1_PSS_PADDING to the maximum permissible value when signing data. */
    const _RSA_PSS_SALTLEN_MAX_SIGN: number;
    /** Causes the salt length for RSA_PKCS1_PSS_PADDING to be determined automatically when verifying a signature. */
    const _RSA_PSS_SALTLEN_AUTO: number;
    const _POINT_CONVERSION_COMPRESSED: number;
    const _POINT_CONVERSION_UNCOMPRESSED: number;
    const _POINT_CONVERSION_HYBRID: number;
    /** Specifies the built-in default cipher list used by Node.js (colon-separated values). */
    const _defaultCoreCipherList: string;
    /** Specifies the active default cipher list used by the current Node.js process  (colon-separated values). */
    const _defaultCipherList: string;
  }
  ...
  /** @deprecated since v10.0.0 */
  const _DEFAULT_ENCODING: BufferEncoding;
   ...
   * @since v17.4.0
   */
  const _subtle: webcrypto.SubtleCrypto;
  /**
