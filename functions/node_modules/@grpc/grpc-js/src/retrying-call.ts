...
const _TRACER_NAME = 'retrying_call';
...
export class RetryingCall implements Call {
  ...
  private trace(text: string): void {
    logging.trace(LogVerbosity.DEBUG, TRACER_NAME, '[' + this.callNumber + '] ' + text);
  }
  ...
  private commitCall(index: number) {
    ...
    this.trace(
      'Committing call [' + this.underlyingCalls[index].call.getCallNumber() + '] at index ' + index
    );
  ...
  private isStatusCodeInList(list: (Status | string)[], code: Status) {
    return list.some(
      (value) => value === code || value.toString().toLowerCase() === Status[code].toLowerCase()
    );
  }
  ...
  private getNextRetryBackoffMs() {
    ...
    const _nextBackoffMs = Math.random() * this.nextRetryBackoffSec * 1000;
    const _maxBackoffSec = Number(
  ...
  private maybeStartHedgingTimer() {
    ...
    const _hedgingDelaySec = Number(hedgingDelayString.substring(0, hedgingDelayString.length - 1));
  ...
  halfClose(): void {
    ...
    const _halfCloseIndex = this.getNextBufferIndex();
