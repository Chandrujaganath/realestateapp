 ...
 */
const _CLIENT_LANGUAGE_STRING = 'node';
...
function validateCanaryConfig(obj: any): ServiceConfigCanaryConfig {
  ...
  const _allowedFields = ['clientLanguage', 'percentage', 'clientHostname', 'serviceConfig'];
...
function validateAndSelectCanaryConfig(obj: any, percentage: number): ServiceConfig {
  ...
  for (const config of obj) {
    ...
    if (Array.isArray(validatedConfig.clientLanguage)) {
      ...
      for (const _language of validatedConfig.clientLanguage) {
...
export function extractAndSelectServiceConfig(
  ...
  for (const record of txtRecord) {
    if (record.length > 0 && record[0].startsWith('grpc_config=')) {
      ...
      const _recordString = record.join('').substring('grpc_config='.length);
      const _recordJson: any = JSON.parse(recordString);
