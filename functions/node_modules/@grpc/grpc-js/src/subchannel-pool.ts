...
import { SubchannelAddress, subchannelAddressEqual } from './subchannel-address';
 ...
 * other references
 */
const _REF_CHECK_INTERVAL = 10_000;
  ...
  unrefUnusedSubchannels(): void {
    ...
    for (const channelTarget in this.pool) {
      ...
      const refedSubchannels = subchannelObjArray.filter(
        (_value) => !value.subchannel.unrefIfOneRef()
      );
    ...
    if (channelTarget in this.pool) {
      ...
      for (const subchannelObj of subchannelObjArray) {
        if (
          ...
          channelOptionsEqual(channelArguments, subchannelObj.channelArguments) &&
...
const _globalSubchannelPool = new SubchannelPool();

/**
 * Get either the global subchannel pool, or a new subchannel pool.
 * @param global
 */
export function getSubchannelPool(_global: boolean): SubchannelPool {
  if (global) {
