 ...
 */
const _CONNECTION_DELAY_INTERVAL_MS = 250;
    ...
    this.subchannelStateListener = (
      ...
      if (newState === ConnectivityState.READY) {
        ...
        if (
          ...
          this.subchannelStateCounts[ConnectivityState.IDLE] === this.subchannels.length
        ) {
  ...
  private connectToAddressList(): void {
    ...
    trace(
      'Connect to address list ' +
        this.latestAddressList.map((address) => subchannelAddressToString(address))
    );
  ...
  updateAddressList(addressList: SubchannelAddress[], lbConfig: LoadBalancingConfig): void {
    ...
    if (
      ...
      !this.latestAddressList.every(
        (_value, index) => addressList[index] && subchannelAddressEqual(addressList[index], value)
      )
    ) {
