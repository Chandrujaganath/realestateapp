...
export class Subchannel {
  ...
  constructor(
    ...
    const _backoffOptions: BackoffOptions = {
  ...
  private handleBackoffTimer() {
    if (this.continueConnecting) {
      ...
      this.transitionToState([ConnectivityState.TRANSIENT_FAILURE], ConnectivityState.IDLE);
    }
  }
  ...
  private startConnectingInternal() {
    ...
    if (options['grpc.keepalive_time_ms']) {
      const _adjustedKeepaliveTime = Math.min(this.keepaliveTime, KEEPALIVE_MAX_TIME_MS);
    ...
    this.connector.connect(this.subchannelAddress, this.credentials, options).then(
      (transport) => {
        if (this.transitionToState([ConnectivityState.CONNECTING], ConnectivityState.READY)) {
          ...
          transport.addDisconnectListener((_tooManyPings) => {
   ...
   */
  private transitionToState(oldStates: ConnectivityState[], newState: ConnectivityState): boolean {
    if (oldStates.indexOf(this.connectivityState) === -1) {
      return false;
    }
    this.trace(ConnectivityState[this.connectivityState] + ' -> ' + ConnectivityState[newState]);
    if (this.channelzEnabled) {
    ...
    const _previousState = this.connectivityState;
