...
const _FLOW_CONTROL_TRACER_NAME = 'transport_flowctrl';

const _clientVersion = require('../../package.json').version;
...
const _tooManyPingsData: Buffer = Buffer.from('too_many_pings', 'ascii');
...
class Http2Transport implements Transport {
  ...
  constructor(
    ...
    session.once('goaway', (errorCode: number, _lastStreamID: number, _opaqueData: Buffer) => {
  ...
  private maybeSendPing() {
    ...
    try {
      this.session!.ping((_err: Error | null, _duration: number, _payload: Buffer) => {
...
export class Http2SubchannelConnector implements SubchannelConnector {
  ...
  private createSession(
    ...
    return new Promise<Http2Transport>((_resolve, reject) => {
      ...
      if ('secureContext' in connectionOptions) {
        ...
        if (options['grpc.ssl_target_name_override']) {
          ...
          const _authorityHostname = splitHostPort(targetAuthority)?.host ?? 'localhost';
        ...
        if (proxyConnectionResult.socket) {
          ...
          connectionOptions.createConnection = (_authority, _option) => {
  ...
  connect(
    ...
    const connectionOptions: ConnectionOptions = credentials._getConnectionOptions() || {};
    ...
    if ('secureContext' in connectionOptions) {
      ...
      if (options['grpc.ssl_target_name_override']) {
        ...
        if ('grpc.http_connect_target' in options) {
          ...
          const _hostPort = splitHostPort(targetPath);
          connectionOptions.servername = hostPort?.host ?? targetPath;
        }
      }
    }

    return getProxiedConnection(address, options, connectionOptions).then((_result) =>
      this.createSession(address, credentials, options, result)
    );
  }
