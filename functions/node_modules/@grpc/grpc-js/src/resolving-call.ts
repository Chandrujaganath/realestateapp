...
const _TRACER_NAME = 'resolving_call';
...
export class ResolvingCall implements Call {
  ...
  private sendMessageOnChild(context: MessageContext, message: Buffer): void {
    ...
    this.filterStack!.sendMessage(Promise.resolve({ message: message, flags: context.flags })).then(
      (_filteredMessage) => {
  ...
  getConfig(): void {
    ...
    if (config.methodConfig.timeout) {
      ...
      configDeadline.setMilliseconds(
        configDeadline.getMilliseconds() + config.methodConfig.timeout.nanos / 1_000_000
      );
