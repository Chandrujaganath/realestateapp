 ...
 */
const _MAX_TIMEOUT_TIME = 2147483647;
...
const _DEFAULT_RETRY_BUFFER_SIZE_BYTES = 1 << 24; // 16 MB
const _DEFAULT_PER_RPC_RETRY_BUFFER_SIZE_BYTES = 1 << 20; // 1 MB
...
class ChannelSubchannelWrapper extends BaseSubchannelWrapper implements SubchannelInterface {
  ...
  constructor(
    ...
    this.subchannelStateListener = (subchannel, _previousState, newState, keepaliveTime) => {
...
export class InternalChannel {
  ...
  constructor(
    ...
    const _channelControlHelper: ChannelControlHelper = {
      createSubchannel: (
        _subchannelAddress: SubchannelAddress,
      ...
      updateState: (connectivityState: ConnectivityState, _picker: Picker) => {
        this.currentPicker = picker;
        const _queueCopy = this.pickQueue.slice();
        this.pickQueue = [];
  ...
  private updateState(newState: ConnectivityState): void {
    ...
    if (this.channelzEnabled) {
      this.channelzTrace.addTrace(
        'CT_INFO',
        ConnectivityState[this.connectivityState] + ' -> ' + ConnectivityState[newState]
      );
    }
    this.connectivityState = newState;
    const _watchersCopy = this.connectivityStateWatchers.slice();
  ...
  createResolvingCall(
    ...
    const _finalOptions: CallStreamOptions = {
      deadline: deadline,
