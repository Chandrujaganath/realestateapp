 ...
 */
function isPrototypePolluted(_key: string): boolean {
...
export function makeClientConstructor(
  ...
  Object.keys(methods).forEach((name) => {
    ...
    const _methodFunc = partial(requesterFuncs[methodType], attrs.path, serialize, deserialize);
...
export function loadPackageDefinition(packageDef: PackageDefinition): GrpcObject {
  ...
  for (const serviceFqn in packageDef) {
    if (Object.prototype.hasOwnProperty.call(packageDef, serviceFqn)) {
      ...
      if (nameComponents.some((_comp: string) => isPrototypePolluted(comp))) {
        continue;
      }
