...
export class ChannelzChildrenTracker {
  private channelChildren: Map<number, { ref: ChannelRef; count: number }> = new Map<
    number,
    { ref: ChannelRef; count: number }
  >();
  private subchannelChildren: Map<number, { ref: SubchannelRef; count: number }> = new Map<
    number,
    { ref: SubchannelRef; count: number }
  >();
  private socketChildren: Map<number, { ref: SocketRef; count: number }> = new Map<
    number,
 ...
 */
function parseIPv6Section(_addressSection: string): [number, number] {
...
function parseIPv6Chunk(addressChunk: string): number[] {
  ...
  const _bytePairs = addressChunk.split(':').map((_section) => parseIPv6Section(section));
...
function ipAddressStringToBuffer(ipAddress: string): Buffer | null {
  if (isIPv4(ipAddress)) {
    return Buffer.from(
      Uint8Array.from(ipAddress.split('.').map((_segment) => Number.parseInt(segment)))
    );
    ...
    const _middleBuffer = Buffer.alloc(16 - leftBuffer.length - rightBuffer.length, 0);
...
function GetSubchannel(
  ...
  const _subchannelMessage: SubchannelMessage = {
...
function GetSocket(
  ...
  const _securityMessage: Security | null = resolvedInfo.security
    ? {
        model: 'tls',
        tls: {
    ...
    : null;
  const _socketMessage: SocketMessage = {
...
function GetServerSockets(
  call: ServerUnaryCall<GetServerSocketsRequest__Output, GetServerSocketsResponse>,
  ...
  const _startId = Number.parseInt(call.request.start_socket_id);
  const maxResults = Number.parseInt(call.request.max_results);
  const resolvedInfo = serverEntry.getInfo();
  // If we wanted to include listener sockets in the result, this line would
  // instead say
  // const allSockets = resolvedInfo.listenerChildren.sockets.concat(resolvedInfo.sessionChildren.sockets).sort((ref1, ref2) => ref1.id - ref2.id);
  const allSockets = resolvedInfo.sessionChildren.sockets.sort((_ref1, _ref2) => ref1.id - ref2.id);
...
export function getChannelzServiceDefinition(): ChannelzDefinition {
  ...
  const _loaderLoadSync = require('@grpc/proto-loader').loadSync as typeof loadSync;
  const _loadedProto = loaderLoadSync('channelz.proto', {
    keepCase: true,
    longs: String,
    enums: String,
    defaults: true,
  ...
  const _channelzGrpcObject = loadPackageDefinition(
    loadedProto
