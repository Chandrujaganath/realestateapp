...
const _LEGAL_KEY_REGEX = /^[0-9a-z_.-]+$/;
const _LEGAL_NON_BINARY_VALUE_REGEX = /^[ -~]*$/;
...
export class Metadata {
  ...
  clone(): Metadata {
    ...
    const _newInternalRepr = newMetadata.internalRepr;

    for (const [key, value] of this.internalRepr) {
      const _clonedValue: MetadataValue[] = value.map((v) => {
  ...
  merge(other: Metadata): void {
    for (const [key, values] of other.internalRepr) {
      const _mergedValue: MetadataValue[] = (this.internalRepr.get(key) || []).concat(values);
  ...
  static fromHttp2Headers(headers: http2.IncomingHttpHeaders): Metadata {
    ...
    for (const key of Object.keys(headers)) {
      ...
      try {
        ...
        const _message = `Failed to add metadata entry ${key}: ${values}. ${getErrorMessage(error)}. For more information see https://github.com/grpc/grpc-node/issues/1173`;
...
const _bufToString = (val: string | Buffer): string => {
