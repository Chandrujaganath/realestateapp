...
const _KEEPALIVE_MAX_TIME_MS = ~(1 << 31);
const _KEEPALIVE_TIMEOUT_MS = 20000;

const { HTTP2_HEADER_PATH } = http2.constants;

const _TRACER_NAME = 'server';
...
function getUnimplementedStatusResponse(methodName: string): Partial<ServiceError> {
  return {
...
function getDefaultHandler(handlerType: HandlerType, methodName: string) {
  ...
  switch (handlerType) {
    ...
    case 'clientStream':
      return (call: ServerReadableStream<any, any>, callback: sendUnaryData<any>) => {
        callback(unimplementedStatusResponse as ServiceError, null);
      };
    case 'serverStream':
...
export class Server {
  ...
  private handlers: Map<string, UntypedHandler> = new Map<string, UntypedHandler>();
  ...
  constructor(options?: ChannelOptions) {
    ...
    this.maxConnectionAgeGraceMs =
      this.options['grpc.max_connection_age_grace_ms'] ?? UNLIMITED_CONNECTION_AGE_MS;
  ...
  private getChannelzSessionInfoGetter(session: http2.ServerHttp2Session): () => SocketInfo {
    return () => {
      ...
      const remoteAddress = sessionSocket.remoteAddress
        ? stringToSubchannelAddress(sessionSocket.remoteAddress, sessionSocket.remotePort)
        : null;
  ...
  bindAsync(
    ...
    const setupServer = (): http2.Http2Server | http2.Http2SecureServer => {
      ...
      if (creds._isSecure()) {
        const _secureServerOptions = Object.assign(serverOptions, creds._getSettings()!);
    ...
    const bindSpecificPort = (
      ...
      return Promise.all(
        addressList.map((address) => {
          ...
          return new Promise<number | Error>((resolve, _reject) => {
    ...
    const bindWildcardPort = (addressList: SubchannelAddress[]): Promise<BindResult> => {
      ...
      const http2Server = setupServer();
      return new Promise<BindResult>((resolve, _reject) => {
        const onError = (err: Error) => {
          this.trace(
            'Failed to bind ' + subchannelAddressToString(address) + ' with error ' + err.message
          );
    ...
    const _resolver = createResolver(portUri, resolverListener, this.options);
  ...
  forceShutdown(): void {
    ...
    this.sessions.forEach((_channelzInfo, session) => {
  ...
  private _runHandlerForCall(
    ...
    if (type === 'unary') {
      ...
      handleClientStreaming(call, handler as UntypedClientStreamingHandler, metadata, encoding);
    } else if (type === 'serverStream') {
      handleServerStreaming(call, handler as UntypedServerStreamingHandler, metadata, encoding);
    } else if (type === 'bidi') {
      handleBidiStreaming(call, handler as UntypedBidiStreamingHandler, metadata, encoding);
    } else {
      return false;
    }
  ...
  private _setupHandlers(http2Server: http2.Http2Server | http2.Http2SecureServer): void {
    ...
    http2Server.on('session', (session) => {
      ...
      if (this.maxConnectionAgeMs !== UNLIMITED_CONNECTION_AGE_MS) {
        ...
        const _jitter = Math.random() * jitterMagnitude * 2 - jitterMagnitude;
...
function handleUnary<RequestType, ResponseType>(
  ...
  call.receiveUnaryMessage(encoding, (err, request) => {
    ...
    const _emitter = new ServerUnaryCallImpl<RequestType, ResponseType>(call, metadata, request);
