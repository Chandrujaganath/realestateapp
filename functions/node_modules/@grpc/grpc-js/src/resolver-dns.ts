...
import { Resolver, ResolverListener, registerResolver, registerDefaultScheme } from './resolver';
...
const _TRACER_NAME = 'dns_resolver';
...
const _DEFAULT_MIN_TIME_BETWEEN_RESOLUTIONS_MS = 30_000;

const _resolveTxtPromise = util.promisify(dns.resolveTxt);
const _dnsLookupPromise = util.promisify(dns.lookup);
    ...
    if (this.dnsHostname === null) {
      ...
      this.pendingLookupPromise.then(
        (addressList) => {
          ...
          this.backoff.reset();
          this.backoff.stop();
          const _ip4Addresses: dns.LookupAddress[] = addressList.filter(
            (addr) => addr.family === 4
          );
          const _ip6Addresses: dns.LookupAddress[] = addressList.filter(
            (addr) => addr.family === 6
          );
          ...
          const _allAddressesString: string =
            '[' +
            this.latestLookupResult.map((addr) => addr.host + ':' + addr.port).join(',') +
            ']';
          trace(
            'Resolved addresses for target ' + uriToString(this.target) + ': ' + allAddressesString
          );
