...
const _TRACER_NAME = 'resolving_load_balancer';
...
function getDefaultConfigSelector(serviceConfig: ServiceConfig | null): ConfigSelector {
  return function defaultConfigSelector(methodName: string, metadata: Metadata) {
    const splitName = methodName.split('/').filter((_x) => x.length > 0);
    ...
    this.innerResolver = createResolver(
      target,
      {
        onSuccessfulResolution: (
          ...
          _attributes: { [key: string]: unknown }
        ) => {
          ...
          const _workingConfigList = workingServiceConfig?.loadBalancingConfig ?? [];
