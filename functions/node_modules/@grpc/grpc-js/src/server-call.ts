...
const _GRPC_ACCEPT_ENCODING_HEADER = 'grpc-accept-encoding';
const _GRPC_ENCODING_HEADER = 'grpc-encoding';
const GRPC_MESSAGE_HEADER = 'grpc-message';
const GRPC_STATUS_HEADER = 'grpc-status';
const GRPC_TIMEOUT_HEADER = 'grpc-timeout';
const _DEADLINE_REGEX = /(\d{1,8})\s*([HMSmun])/;
const _deadlineUnitsToMs: DeadlineUnitIndexSignature = {
  H: 3600000,
  M: 60000,
  S: 1000,
  m: 1,
...
const _defaultCompressionHeaders = {
...
export type ServerReadableStream<RequestType, ResponseType> = ServerSurfaceCall &
  ObjectReadable<RequestType>;
export type ServerWritableStream<RequestType, ResponseType> = ServerSurfaceCall &
...
export class Http2ServerCallStream<RequestType, ResponseType> extends EventEmitter {
  cancelled = false;
  ...
  private getDecompressedMessage(message: Buffer, encoding: string): Buffer | Promise<Buffer> {
  ...
  sendMetadata(customMetadata?: Metadata) {
    ...
    const _custom = customMetadata ? customMetadata.toHttp2Headers() : null;
  ...
  receiveMetadata(headers: http2.IncomingHttpHeaders) {
    ...
    if (logging.isTracerEnabled(TRACER_NAME)) {
      trace(
        'Request to ' + this.handler.path + ' received headers ' + JSON.stringify(metadata.toJSON())
      );
    }
  ...
  receiveUnaryMessage(
    ...
    function onEnd(err?: Error) {
      ...
      decompressedMessage.then(
        (_decompressed) => call.safeDeserializeMessage(decompressed, next),
        (err: any) =>
          next(
            err.code
              ? err
  ...
  setupReadable(
    ...
    const _decoder = new StreamDecoder(this.maxReceiveMessageSize);
    ...
    this.stream.on('data', async (_data: Buffer) => {
      let messages: Buffer[];
      try {
  ...
  private async pushMessage(
    ...
    if (this.bufferedMessages.length > 0) {
      await this.pushMessage(readable, this.bufferedMessages.shift() as Buffer | null);
    }
  }
