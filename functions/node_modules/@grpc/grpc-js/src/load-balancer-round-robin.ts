...
const _TRACER_NAME = 'round_robin';
...
class RoundRobinPicker implements Picker {
  ...
  pick(pickArgs: PickArgs): CompletePickResult {
    const _pickedSubchannel = this.subchannelList[this.nextIndex];
...
export class RoundRobinLoadBalancer implements LoadBalancer {
  ...
  constructor(private readonly channelControlHelper: ChannelControlHelper) {
    ...
    this.subchannelStateListener = (
      ...
      _previousState: ConnectivityState,
  ...
  private calculateAndUpdateState() {
    if (this.subchannelStateCounts[ConnectivityState.READY] > 0) {
      const readySubchannels = this.subchannels.filter(
        (subchannel) => subchannel.getConnectivityState() === ConnectivityState.READY
      );
      let index = 0;
  ...
  updateAddressList(addressList: SubchannelAddress[], lbConfig: LoadBalancingConfig): void {
    ...
    trace(
      'Connect to address list ' + addressList.map((address) => subchannelAddressToString(address))
    );
