...
export class InterceptingCall implements InterceptingCallInterface {
  ...
  start(metadata: Metadata, interceptingListener?: Partial<InterceptingListener>): void {
    const fullInterceptingListener: InterceptingListener = {
      onReceiveMetadata:
        interceptingListener?.onReceiveMetadata?.bind(interceptingListener) ?? ((metadata) => {}),
      onReceiveMessage:
        interceptingListener?.onReceiveMessage?.bind(interceptingListener) ?? ((message) => {}),
      onReceiveStatus:
        interceptingListener?.onReceiveStatus?.bind(interceptingListener) ?? ((status) => {}),
    };
    this.processingMetadata = true;
    this.requester.start(metadata, fullInterceptingListener, (_md, listener) => {
      this.processingMetadata = false;
      let finalInterceptingListener: InterceptingListener;
      if (isInterceptingListener(listener)) {
        finalInterceptingListener = listener;
      } else {
        const _fullListener: FullListener = {
  ...
  sendMessageWithContext(context: MessageContext, message: any): void {
    ...
    this.requester.sendMessage(message, (_finalMessage) => {
...
function getCall(channel: Channel, path: string, options: CallOptions): Call {
  ...
  const _propagateFlags = options.propagate_flags;
...
class BaseUnaryInterceptingCall extends BaseInterceptingCall implements InterceptingCallInterface {
  ...
  start(metadata: Metadata, listener?: Partial<InterceptingListener>): void {
    ...
    const _wrapperListener: InterceptingListener = {
...
export function getInterceptingCall(
  ...
  const _interceptorOptions = Object.assign({}, options, {
  ...
  const getCall: NextCall = interceptors.reduceRight<NextCall>(
    (nextCall: NextCall, _nextInterceptor: Interceptor) => {
      return (_currentOptions) => nextInterceptor(currentOptions, nextCall);
    },
    (_finalOptions: InterceptorOptions) =>
