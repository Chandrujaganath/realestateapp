...
function isCurrentOauth2Client(client: OAuth2Client): client is CurrentOAuth2Client {
...
class ComposedCallCredentials extends CallCredentials {
  ...
  async generateMetadata(options: CallMetadataOptions): Promise<Metadata> {
    ...
    const _generated: Metadata[] = await Promise.all(
      this.creds.map((_cred) => cred.generateMetadata(options))
    );
    for (const _gen of generated) {
      base.merge(gen);
    }
    return base;
  }
  ...
  _equals(other: CallCredentials): boolean {
    ...
    if (other instanceof ComposedCallCredentials) {
      return this.creds.every((_value, index) => value._equals(other.creds[index]));
    } else {
      return false;
    }
  }
