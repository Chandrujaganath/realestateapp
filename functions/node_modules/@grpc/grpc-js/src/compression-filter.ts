...
const _isCompressionAlgorithmKey = (key: number): key is CompressionAlgorithms => {
...
abstract class CompressionHandler {
  ...
  async readMessage(data: Buffer): Promise<Buffer> {
    const _compressed = data.readUInt8(0) === 1;
...
export class CompressionFilter extends BaseFilter implements Filter {
  ...
  constructor(
    ...
    this.maxReceiveMessageLength =
      channelOptions['grpc.max_receive_message_length'] ?? DEFAULT_MAX_RECEIVE_MESSAGE_LENGTH;
  ...
  receiveMetadata(metadata: Metadata): Metadata {
    ...
    const serverSupportedEncodingsHeader = metadata.get('grpc-accept-encoding')[0] as
      | string
      | undefined;
