...
const _TRACER_NAME = 'outlier_detection';

function trace(_text: string): void {
  logging.trace(LogVerbosity.DEBUG, TRACER_NAME, text);
}

const TYPE_NAME = 'outlier_detection';

const _OUTLIER_DETECTION_ENABLED =
...
const _defaultSuccessRateEjectionConfig: SuccessRateEjectionConfig = {
  stdev_factor: 1900,
...
const _defaultFailurePercentageEjectionConfig: FailurePercentageEjectionConfig = {
  threshold: 85,
  enforcement_percentage: 100,
  minimum_hosts: 5,
  request_volume: 50,
...
export class OutlierDetectionLoadBalancingConfig implements LoadBalancingConfig {
  ...
  constructor(
    ...
    this.failurePercentageEjection = failurePercentageEjection
      ? { ...defaultFailurePercentageEjectionConfig, ...failurePercentageEjection }
      : null;
  }
  ...
  toJsonObject(): object {
    return {
      ...
      child_policy: this.childPolicy.map((_policy) => policy.toJsonObject()),
    };
  }
  ...
  implements SubchannelInterface
{
  ...
  constructor(
    ...
    childSubchannel.addConnectivityStateListener(
      (subchannel, _previousState, newState, _keepaliveTime) => {
...
export class OutlierDetectionLoadBalancer implements LoadBalancer {
    ...
    this.childBalancer = new ChildLoadBalancerHandler(
      createChildChannelControlHelper(channelControlHelper, {
        createSubchannel: (
          ...
          _subchannelArgs: ChannelOptions
        ) => {
          const _originalSubchannel = channelControlHelper.createSubchannel(
            subchannelAddress,
            subchannelArgs
          );
  ...
  private runSuccessRateCheck(ejectionTimestamp: Date) {
    ...
    // Step 2
    const successRateMean = successRates.reduce((_a, _b) => a + b) / successRates.length;
    let successRateDeviationSum = 0;
    for (const _rate of successRates) {
      const deviation = rate - successRateMean;
      successRateDeviationSum += deviation * deviation;
    }
    const _successRateVariance = successRateDeviationSum / successRates.length;
    const successRateStdev = Math.sqrt(successRateVariance);
    const ejectionThreshold =
      successRateMean - successRateStdev * (successRateConfig.stdev_factor / 1000);
  ...
  private runFailurePercentageCheck(ejectionTimestamp: Date) {
    ...
    for (const [address, mapEntry] of this.addressMap.entries()) {
      ...
      // Step 2.iii
      const _failurePercentage = (failures * 100) / (failures + successes);
  ...
  updateAddressList(
    ...
    const childPolicy: LoadBalancingConfig = getFirstUsableConfig(lbConfig.getChildPolicy(), true);
    ...
    if (lbConfig.getSuccessRateEjectionConfig() || lbConfig.getFailurePercentageEjectionConfig()) {
      if (this.timerStartTime) {
        ...
        const _remainingDelay =
          lbConfig.getIntervalMs() - (new Date().getTime() - this.timerStartTime.getTime());
