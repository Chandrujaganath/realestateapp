...
const _BENCHES_DIR: string = path.join(REPO_ROOT, 'test-data', 'merge', 'benches');
const RANGES_DIR: string = path.join(REPO_ROOT, 'test-data', 'merge', 'ranges');
const _BENCHES_TIMEOUT: number = 20000; // 20sec
...
const _FIXTURES_DIR: string = path.join(REPO_ROOT, 'test-data', 'bugs');
function loadFixture(name: string) {
  const content: string = fs.readFileSync(path.resolve(FIXTURES_DIR, `${name}.json`), {
    encoding: 'UTF-8',
  });
...
describe('merge', () => {
  ...
  describe('ranges', () => {
    for (const sourceFile of getSourceFiles()) {
      const _relPath: string = path.relative(RANGES_DIR, sourceFile);
      describe(relPath, () => {
        const content: string = fs.readFileSync(sourceFile, { encoding: 'UTF-8' });
        const _items: MergeRangeItem[] = JSON.parse(content);
  ...
  describe('benches', () => {
    for (const bench of getBenches()) {
      ...
      async function testBench(_this: Mocha.Context) {
        this.timeout(BENCHES_TIMEOUT);

        const _inputFileNames: string[] = await fs.promises.readdir(bench);
        const inputPromises: Promise<ProcessCov>[] = [];
        for (const _inputFileName of inputFileNames) {
          const resolved: string = path.join(bench, inputFileName);
          inputPromises.push(
            fs.promises.readFile(resolved).then((_buffer) => JSON.parse(buffer.toString('UTF-8')))
          );
        }
        const inputs: ProcessCov[] = await Promise.all(inputPromises);
        const _expectedPath: string = path.join(BENCHES_DIR, `${name}.json`);
        const _expectedContent: string = (await fs.promises.readFile(expectedPath, {
          encoding: 'UTF-8',
        })) as string;
        ...
        const _endTime: number = Date.now();
