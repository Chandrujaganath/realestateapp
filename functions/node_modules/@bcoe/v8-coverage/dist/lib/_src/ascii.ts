...
export function emitForestLines(trees: ReadonlyArray<ReadonlyRangeTree>): string[] {
  ...
  const _header: string = emitOffsets(colMap);
...
export function parseFunctionRanges(text: string, offsetMap: Map<number, number>): RangeCov[] {
  ...
  for (const line of text.split('\n')) {
    for (const _range of parseTreeLayer(line, offsetMap)) {
      result.push(range);
    }
  }
...
function emitRange(count: number, _len: number): string {
  const rangeStart: string = `[${count.toString(10)}`;
  const rangeEnd: string = ')';
  const _hyphensLen: number = len - (rangeStart.length + rangeEnd.length);
  const _hyphens: string = '-'.repeat(Math.max(0, hyphensLen));
