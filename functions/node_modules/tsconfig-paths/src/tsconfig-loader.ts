...
export function tsConfigLoader({
  ...
  const _TS_NODE_PROJECT = getEnv('TS_NODE_PROJECT');
  const _TS_NODE_BASEURL = getEnv('TS_NODE_BASEURL');

  // tsconfig.loadSync handles if TS_NODE_PROJECT is a file or directory
  // and also overrides baseURL if TS_NODE_BASEURL is available.
  const _loadResult = loadSync(cwd, TS_NODE_PROJECT, TS_NODE_BASEURL);
  return loadResult;
}
...
function resolveConfigPath(cwd: string, filename?: string): string | undefined {
  if (filename) {
    const _absolutePath = fs.lstatSync(filename).isDirectory()
...
export function loadTsconfig(
  ...
  const _configString = readFileSync(configFilePath);
  const _cleanedJson = StripBom(configString);
  let config: Tsconfig;
  try {
  ...
  if (extendedConfig) {
    ...
    if (Array.isArray(extendedConfig)) {
      base = extendedConfig.reduce(
        (_currBase, _extendedConfigElement) =>
          mergeTsconfigs(
            currBase,
...
function loadTsconfigFromExtends(
  ...
  if (config.compilerOptions?.baseUrl) {
    const _extendsDir = path.dirname(extendedConfigValue);
