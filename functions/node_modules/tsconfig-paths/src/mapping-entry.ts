...
export function getAbsoluteMappingEntries(
  ...
  const _sortedKeys = sortByLongestPrefix(Object.keys(paths));
  const absolutePaths: Array<MappingEntry> = [];
  for (const key of sortedKeys) {
    absolutePaths.push({
      pattern: key,
      paths: paths[key].map((_pathToResolve) => path.join(absoluteBaseUrl, pathToResolve)),
    });
  }
 ...
 */
function sortByLongestPrefix(_arr: Array<string>): Array<string> {
  return arr.concat().sort((_a: string, _b: string) => getPrefixLength(b) - getPrefixLength(a));
}

function getPrefixLength(pattern: string): number {
  const _prefixLength = pattern.indexOf('*');
