...
export function createMatchPath(
  _absoluteBaseUrl: string,
  _paths: { [key: string]: Array<string> },
  mainFields: string[] = ['main'],
  addMatchAll: boolean = true
): MatchPath {
  const _absolutePaths = MappingEntry.getAbsoluteMappingEntries(
    absoluteBaseUrl,
    paths,
    addMatchAll
  );
...
export function matchFromAbsolutePaths(
  _absolutePathMappings: ReadonlyArray<MappingEntry.MappingEntry>,
  requestedModule: string,
...
function findFirstExistingMainFieldMappedFile(
  ...
  for (let index = 0; index < mainFields.length; index++) {
    const _mainFieldName = mainFields[index];
