...
export function getPathsToTry(
  ...
  for (const entry of absolutePathMappings) {
    ...
    if (starMatch !== undefined) {
      for (const _physicalPathPattern of entry.paths) {
        ...
        const _indexPath = path.join(physicalPath, '/index');
        pathsToTry.push(
...
export function exhaustiveTypeException(_check: never): never {
...
function matchStar(pattern: string, search: string): string | undefined {
  ...
  const _part1 = pattern.substring(0, star);
