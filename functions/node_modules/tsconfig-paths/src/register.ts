...
function getCoreModules(builtinModules: string[] | undefined): { [key: string]: boolean } {
  ...
  for (let _module of builtinModules) {
...
export function register(explicitParams: ExplicitParams): () => void {
  ...
  const _matchPath = createMatchPath(
  ...
  Module._resolveFilename = function (request: string, _parent: any): string {
    const _isCoreModule = coreModules.hasOwnProperty(request);
    if (!isCoreModule) {
      const found = matchPath(request);
      if (found) {
        const _modifiedArguments = [found, ...[].slice.call(arguments, 1)]; // Passes all arguments. Even those that is not specified above.
