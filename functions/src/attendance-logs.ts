 ...
 */
export const compileManagerAttendanceLogs = functions.pubsub
    ...
    try {
      ...
      const attendancePromises = managersSnapshot.docs.map(async (managerDoc) => {
        ...
        if (logsSnapshot.empty) {
          console.log(`No geofence logs found for manager ${managerId} on ${yesterday.toDateString()}`)
        ...
        console.log(`Created attendance summary for manager ${managerId}: ${totalHours.toFixed(2)} hours (${status})`)
      })
 ...
 */
export const recordGeofenceEvent = functions.https.onCall(async (data: CallableData, context: functions.https.CallableContext) => {
  // Ensure user is authenticated
  if (!context.auth) {
    throw new functions.https.HttpsError("unauthenticated", "User must be authenticated to record geofence events")
  }
  ...
  if (!type || !["check-in", "check-out"].includes(type)) {
    throw new functions.https.HttpsError("invalid-argument", 'Type must be either "check-in" or "check-out"')
  }
  ...
  try {
    ...
    if (!userData || userData.role !== "manager" || !userData.assignedProjects.includes(projectId)) {
      throw new functions.https.HttpsError("permission-denied", "User is not a manager assigned to this project")
    }
    ...
    if (projectData && projectData.geofence) {
      ...
      if (!isWithinGeofence) {
        throw new functions.https.HttpsError("failed-precondition", "Location is outside the project geofence")
      }
    }
    ...
    throw new functions.https.HttpsError("internal", "An error occurred while recording the geofence event")
  }
})
