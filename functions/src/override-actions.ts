...
export const _overrideAction = functions.https.onCall(
  async (data: OverrideData, context: functions.https.CallableContext) => {
...
async function handleProjectOverride(data: OverrideData) {
  ...
  switch (actionType) {
    ...
    case 'REASSIGN_PROJECT_MANAGER':
      ...
      const _newManagerRef = projectRef.collection('managers').doc(newData.managerId);
      ...
      if (newData.oldManagerId) {
        const _oldManagerRef = projectRef.collection('managers').doc(newData.oldManagerId);
