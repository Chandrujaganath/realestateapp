...
export const _assignTask = functions.https.onCall(async (data, context) => {
  try {
    ...
    const _userDoc = await admin.firestore().collection('users').doc(context.auth.uid).get();
    ...
    const _projectDoc = await admin.firestore().collection('projects').doc(projectId).get();
    ...
    if (!assignedManagerId) {
      ...
      const managersWithTaskCount = await Promise.all(
        projectManagers.docs.map(async (managerDoc) => {
          const _activeTasks = await admin
            .firestore()
            .collection('tasks')
      ...
      const _managerWithLeastTasks = managersWithTaskCount.reduce(
