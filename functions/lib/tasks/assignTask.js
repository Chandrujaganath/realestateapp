...
exports.assignTask = functions.https.onCall(async (data, context) => {
    try {
        ...
        if (!userData || (userData.role !== "admin" && userData.role !== "superadmin")) {
            throw new functions.https.HttpsError("permission-denied", "Only admin users can assign tasks");
        }
        // Validate required fields
        if (!title || !projectId || !dueDate) {
            throw new functions.https.HttpsError("invalid-argument", "Missing required fields: title, projectId, and dueDate are required");
        }
        ...
        if (!assignedManagerId) {
            ...
            if (projectManagers.empty) {
                throw new functions.https.HttpsError("failed-precondition", "No active managers found for this project");
            }
            // Find the manager with the least number of active tasks
            const managersWithTaskCount = await Promise.all(projectManagers.docs.map(async (managerDoc) => {
            ...
            const managerWithLeastTasks = managersWithTaskCount.reduce((min, current) => (current.taskCount < min.taskCount ? current : min), managersWithTaskCount[0]);
