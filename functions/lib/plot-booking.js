...
exports.bookPlot = functions.https.onCall(async (data, context) => {
    ...
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "User must be authenticated to book a plot");
    }
    ...
    try {
        ...
        const result = await db.runTransaction(async (transaction) => {
            ...
            if (!plotData || plotData.status !== "available") {
                throw new functions.https.HttpsError("failed-precondition", `Plot is not available for booking. Current status: ${(plotData === null || plotData === void 0 ? void 0 : plotData.status) || "unknown"}`);
            }
            ...
            const bookingData = Object.assign({ id: bookingRef.id, plotId,
                projectId, clientId: userId, clientName: (userData === null || userData === void 0 ? void 0 : userData.displayName) || (userData === null || userData === void 0 ? void 0 : userData.email) || "Unknown Client", status: "pending", createdAt: admin.firestore.FieldValue.serverTimestamp(), updatedAt: admin.firestore.FieldValue.serverTimestamp() }, bookingDetails);
