...
export async function GET(req: Request) {
  try {
    ...
    if (usingDummyImplementation || process.env.NODE_ENV === 'development') {
      console.log(
        'Using mock announcements data because Firebase is unavailable or in development mode'
      );
    ...
    try {
      ...
      const _announcementsSnapshot = await adminDb
      ...
      const announcements = announcementsSnapshot.docs
        .map((doc: DocumentData) => {
          ...
          if (
            ...
            targetRoles.some((_r: string) => r.toLowerCase() === role?.toLowerCase())
          ) {
            return {
              id: doc.id,
              ...data,
    ...
    return NextResponse.json(
      ...
      { status: 200 }
    ); // Return 200 with empty array to prevent UI errors
  }
}
  ...
  try {
    ...
    if (!title || !content || !priority || !targetRoles || !status) {
      return NextResponse.json(
        {
          error:
            'Missing required fields: title, content, priority, targetRoles, and status are required',
        },
        { status: 400 }
      );
    }
