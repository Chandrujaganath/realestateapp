...
export default function AttendancePage() {
  ...
  useEffect(() => {
    const _fetchAttendance = async () => {
      try {
        ...
        const _firstDay = new Date(currentMonth.getFullYear(), currentMonth.getMonth(), 1);
        const lastDay = new Date(currentMonth.getFullYear(), currentMonth.getMonth() + 1, 0);

        let records: AttendanceRecord[] = [];
        if (getManagerAttendance) {
          const fetchedRecords = await getManagerAttendance(_firstDay, lastDay);
          records = fetchedRecords.map((record: any) => ({
            date: record.date,
            status: record.status || 'absent',
            clockInTime: record.clockInTime,
            clockOutTime: record.clockOutTime,
            totalHours: record.totalHours,
            geofenceEvents: record.geofenceEvents || [],
        ...
        const daysToCount = _isCurrentMonth ? now.getDate() : lastDay.getDate();
        ...
        for (let day = 1; day <= daysToCount; day++) {
          ...
          const record = attendanceMap[_dateStr];
        ...
        setSummary({
          ...
          averageHoursPerDay:
            presentCount > 0 ? parseFloat((totalHours / presentCount).toFixed(2)) : 0,
        });
      } catch (error) {
  ...
  const getDayAttendanceStatus = (
    date: Date
  ): 'present' | 'absent' | 'partial' | 'future' | 'none' => {
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            <div className="p-4 bg-muted/50 rounded-md">
              ...
              <p className="text-xs text-muted-foreground mt-1">Average hours per work day</p>
            </div>
          </div>
                      ...
                      getSelectedDayAttendance()?.geofenceEvents?.length > 0 && (
                        <div>
                          ...
                          <div className="space-y-2">
                            {getSelectedDayAttendance()?.geofenceEvents?.map((event, idx) => (
                              <div
                                key={idx}
