...
import { Building2, Calendar, Users, CheckCircle, Clock, ClipboardList, AlertCircle } from "lucide-react"
...
export default function ManagerDashboard() {
  const { user, getManagerTasks, isManagerClockedIn, checkInManager, checkOutManager, isWithinGeofence } = useManagerAuth();
        ...
        if (navigator.geolocation) {
          navigator.geolocation.getCurrentPosition(
            (pos) => {
              ...
              isWithinGeofence(locationData).then(isWithin => setWithinGeofence(isWithin));
            },
            (error) => {
    ...
    try {
      ...
      if (!currentPosition) {
        toast({
          ...
          description: locationError || "Could not determine your location. Check-in may be recorded without location data.",
          variant: "default",
        ...
        const inGeofence = await isWithinGeofence(coords);
        
        if (!inGeofence) {
          // If not in geofence, warn user but still allow check-in
          toast({
            title: "Location Warning",
            description: "You appear to be outside the designated work area. Your check-in will still be recorded, but may be flagged for review.",
            variant: "default",
            className: "bg-amber-50 border-amber-200 text-amber-700"
          });
        }
  ...
  const loadStatusData = useCallback(async () => {
    try {
      ...
      const geoEnabled = await getCurrentLocation()
        .then(pos => {
          if (pos) {
            setPosition(pos);
