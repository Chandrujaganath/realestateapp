...
import { Building2, Calendar, Users, Bell, BarChart3, Settings, CheckCircle, XCircle, Clock } from "lucide-react"
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const analyticsData = await fetch('/api/analytics').then(res => res.json())
        setAnalytics(analyticsData)

        // Fetch pending visits
        if (getVisitRequests) {
          const visits = await getVisitRequests()
          const pendingVisitsList = visits.filter((visit: any) => visit.status === "pending")
          setPendingVisits(pendingVisitsList.length)
        }

        // Fetch pending leave requests
        if (getAllLeaveRequests) {
          const leaves = await getAllLeaveRequests()
          const pendingLeavesList = leaves.filter((leave: any) => leave.status === "pending")
          setPendingLeaves(pendingLeavesList.length)
        }

        // Fetch active projects
        if (getProjects) {
          const projects = await getProjects()
          const active = projects.filter((project: Project) => project.status === "active")
        ...
        if (getAnnouncements) {
          ...
          const active = announcements.filter((a: any) => a.isPublished)
          ...
          <CardContent>
            <div className="space-y-4">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold">
                  {analytics?.managers?.attendanceRate ? Math.round(analytics.managers.attendanceRate * 100) : 0}%
                </span>
              </div>
              <div className="w-full bg-muted rounded-full h-2.5">
                <div
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold">
                  {analytics?.plots?.sold ? Math.round((analytics.plots.sold / analytics.plots.total) * 100) : 0}%
                </span>
              </div>
              <div className="w-full bg-muted rounded-full h-2.5">
                <div
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">Project Sunrise Gardens updated to 75% completion</p>
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">Admin published "New Phase Launch" announcement</p>
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">Admin rejected leave request from Bob Manager</p>
